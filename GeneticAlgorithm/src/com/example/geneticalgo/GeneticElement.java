...
public class GeneticElement implements Comparable<GeneticElement> {
    ...
    public static Double AccurecyFactor = null;
    public static Integer Size = null;

    public GeneticElement() {
        generateRandomly();
        calculateOutputValue();
        calculateFittnesValue();
    }

    public GeneticElement(GeneticElement ge){
        this.fitnessValue = new Double(ge.getFitnessValue());
        this.outputValue = new Double(ge.getOutputValue());
    ...
    private void copyNumbers(ArrayList<Integer> numbers){
        ...
        for(int i = 0; i < GeneticElement.Size; i++){
            this.numbers.add(i, new Integer(numbers.get(i)));
        }
    }
    ...
    private void generateRandomly() {
        ...
        numbers.add(0, new Integer(rand.nextInt(10)));
        ...
        for (int i = 1; i < GeneticElement.Size; ++i) {
            ...
            while (signToAdd == Sign.Devide && numToAdd == 0) {
                numToAdd = rand.nextInt(10);
            }
    ...
    public boolean isElementValid() {
        ...
        if (firstSign == Sign.Devide || firstSign == Sign.Multiply) {
            isValid = false;
        }

        for (int i = 0; (i < GeneticElement.Size) && isValid; ++i) {
            if ((signs.get(i) == Sign.Devide) && (numbers.get(i) == 0)) {
                isValid = false;
            }
        }
    ...
    private void calculateOutputValue() {
        ...
        this.outputValue = new Double(res);
    }

    private void calculateFittnesValue() {
        double dist = Math.abs(this.outputValue - GeneticElement.TargetValue);
        this.fitnessValue = (dist > GeneticElement.AccurecyFactor) ? (1 / dist) : (1 / GeneticElement.AccurecyFactor);
    }
    ...
    public void mutate() {
        ...
        do {
            ...
            numbers.add(index, new Integer(tempNum));
            signs.add(index, tempSign);

        } while (tempSign == Sign.Devide && tempNum == 0);
    ...
    public static void CrossOver(GeneticElement e1, GeneticElement e2) {
        ...
        e1.calculateFittnesValue();

        e2.calculateOutputValue();
        e2.calculateFittnesValue();
    }
